...
//	echo "<td> ". $row['itemcallnumber'] ." </td>";
	...
	if ($row['author'] == NULL)
		{
		echo "<td> ";
		echo "<td><center><i> --group-- ". substr($mrc,0,strlen($mrc)) ."</i></center></td>";
		$marcauth = getmarctag($row['marc'], "110");
		if ( ! $marcauth )
			echo "<b><i>Various Artists</i></b>";
		else
			echo "<b><i>$marcauth</i></b>";
		echo " </td>";
		}
	else
	...
	if ($row['multivolumepart'] != NULL && substr($row['multivolumepart'],0,1) != "1_" )
